...
public final class UnorderedConsumerVerticle extends ConsumerVerticle {
    ...
    private void handleRecords(final ConsumerRecords<Object, CloudEvent> records) {
        ...
        for (var record : records) {
            ...
            // Handle skipped offsets first, we dispatch an InvalidCloudEvent in stead of the missing offsets
            if (lastOffsets.containsKey(topicPartition)) {
                for (long skipOffset = lastOffsets.get(topicPartition) + 1; skipOffset < record.offset(); skipOffset++) {
                  recordDispatcherFutures.add(this.recordDispatcher.dispatch(new ConsumerRecord<>(record.topic(), record.partition(), skipOffset, null, new InvalidCloudEvent(new byte[]{}))));
                }
            }
